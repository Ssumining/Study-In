  ...
  return (
    <div className="flex flex-col gap-10 bg-background">
      <div className="flex flex-col border border-gray-300 rounded-xl overflow-hidden">
        ...
        {/* 상단: 프로필 이미지 + 닉네임 + 소개 */}
        <div className="flex flex-col items-center gap-[30px] px-[195px] py-10 border-b border-gray-300">
          <div className="flex flex-col items-center gap-5">
            ...
            <h2 className="text-lg font-bold text-surface text-center">{profile.nickname}</h2>
          </div>
          <p className="text-sm text-surface text-center bg-gray-100 rounded-[10px] px-[30px] py-5 w-full min-h-[80px] flex items-center justify-center">
            {profile.introduction || '소개글이 없어요.'}
          </p>
        </div>

        {/* 하단: 정보 */}
        <div className="flex flex-col gap-[30px] px-10 py-[30px]">
          <div className="flex flex-col gap-5">

            <div className="flex items-center gap-[30px]">
              <span className="text-base font-bold text-gray-700 w-[70px] shrink-0">이메일(ID)</span>
              <span className="text-lg text-gray-700">{storage.getEmail() ?? ''}</span>
            </div>

            {isMyProfile && profile.name && (
              <div className="flex items-center gap-[30px]">
                <span className="text-base font-bold text-gray-700 w-[70px] shrink-0">이름</span>
            ...
            {isMyProfile && profile.phone && (
              <div className="flex items-center gap-[30px]">
                <span className="text-base font-bold text-gray-700 w-[70px] shrink-0">전화번호</span>
            ...
            {profile.preferred_region && (
              <div className="flex items-center gap-[30px]">
                <span className="text-base font-bold text-gray-700 w-[70px] shrink-0">내 지역</span>
            ...
            {profile.github_username && (
              <>
                <div className="flex items-center gap-[30px]">
                  <span className="text-base font-bold text-gray-700 w-[70px] shrink-0">GitHub</span>
          ...
          {profile.tag.length > 0 && (
            <div className="flex items-center gap-[30px]">
              <span className="text-base font-bold text-gray-700 shrink-0">관심 분야</span>
              <div className="flex flex-wrap gap-[10px]">
                {profile.tag.map((tag) => (
                  <span
                    key={tag.id}
                    className="bg-primary-light text-background text-base px-[14px] py-[6px] rounded-[36px]"
                  >
                    {tag.name}
                  </span>
                ))}
