  ...
  if (!isLoggedIn) {
    return (
      <div className="w-[280px] h-[300px] bg-gray-50 rounded-[12px] flex flex-col items-center justify-center p-6 border border-gray-100">
        ...
        <p className="text-gray-600 text-sm mb-6 text-center">스터디를 만들어<br/>사람들과 함께 공부할 수 있어요!</p>
        <button 
  ...
  return (
    <div className="flex flex-col gap-6 w-[280px]">
      ...
      <div className="flex flex-col gap-3">
        <h4 className="font-bold text-lg">참여 중인 스터디 <span className="text-primary">{studies.length}개</span></h4>
        
        {studies.length > 0 ? (
          // 스터디가 있는 경우 (첫 번째 사진 상황)
          <div className="flex flex-col gap-2 p-4 bg-white border border-gray-100 rounded-[12px]">
            {studies.slice(0, 4).map((study) => (
              <div key={study.id} className="flex gap-3 py-2 border-b last:border-0 border-gray-50">
                <img src={study.image} className="w-12 h-12 rounded-[8px] object-cover" alt="스터디" />
          ...
          <div className="py-10 text-center bg-white border border-dashed border-gray-200 rounded-[12px]">
            <p className="text-gray-400 text-sm">아직 참여 중인 스터디가 없어요.</p>
          </div>
        )}
      </div>
    </div>
